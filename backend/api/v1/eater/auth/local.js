...
// Eater register with email and phone
passport.use('eater-register-email-phone', new LocalStrategy(
    {
        usernameField: 'email',
        passwordField: 'password',
        passReqToCallback: true   // uncomment this to access req in the callback (possibly to get other attributes from req.body)
            ...
            if (emailUsed)
                ...
                database.eaterPhoneExists(phone, (err, phoneUsed) => {
                    ...
                    if (phoneUsed)
                        ...
                        // since email and phone are not used, we can create the Eater account
                        // using bcrypt to hash the password
                        bcrypt.hash(password, BCRYPT_SALT_ROUNDS).then(async hashedPassword => {
                            database.eaterRegister(email, phone, hashedPassword, f_name, l_name, (err, id) => {
                                if (err) return done(err);
                                return done(null, id);
                            });
                        }).catch(err => { return done(err); })
